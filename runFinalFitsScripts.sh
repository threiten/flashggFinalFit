...
if [ $CONTINUELOOP == 0 ]; then
	...
	if [ $NOSYSTS == 1 ]; then
		...
		echo "./runSignalScripts.sh -i $FILE -p $PROCS -f $CATS --ext $EXT --intLumi $INTLUMI $BATCHOPTION --scalesCorr $SCALESCORR --scalesGlobal $SCALESGLOBAL --bs $BS $MHREFOPT --sigFitOnly --fTestOnly --sigPlotsOnly  --keepCurrentFits --noSyts $RUNSIGOPT"
       		./runSignalScripts.sh -i $FILE -p $PROCS -f $CATS --ext $EXT --intLumi $INTLUMI $BATCHOPTION --smears $SMEARS --scales $SCALES --scalesCorr $SCALESCORR --scalesGlobal $SCALESGLOBAL --bs $BS $MHREFOPT --packageOnly --sigFitOnly --fTestOnly --sigPlotsOnly --keepCurrentFits --noSysts $RUNSIGOPT
	    else
		echo "------------------------------------------------"
		...
		./runSignalScripts.sh -i $FILE -p $PROCS -f $CATS --ext $EXT --intLumi $INTLUMI $BATCHOPTION --smears $SMEARS --scales $SCALES --scalesCorr $SCALESCORR --scalesGlobal $SCALESGLOBAL --bs $BS $MHREFOPT --packageOnly --sigFitOnly --fTestOnly --sigPlotsOnly --noSysts $RUNSIGOPT
	    fi
	else
	    echo "------------------------------------------------"
